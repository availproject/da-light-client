...
const lib = ffi.Library('libverifier', {
    verify_proof: ['bool', ['uint8', 'uint8', U8Array, 'size_t', U8Array, 'size_t']],
})

module.exports = {

    verifyProof: (row, col, commitment, proof) => {

        const _commitment = new U8Array(commitment)
        const _proof = new U8Array(proof)

        return lib.verify_proof(row, col, _commitment, _commitment.length, _proof, _proof.length)
